 echo '# Use powerline' >> .zshrc
 echo 'USE_POWERLINE="true"' >> .zshrc
 echo '# Has weird character width' >> .zshrc
 echo '# Example:' >> .zshrc
 echo '#  is not a diamond' >> .zshrc
 echo 'HAS_WIDECHARS="false"' >> .zshrc
 echo 'export TERM=xterm-256color' >> .zshrc
 echo '# Source manjaro-zsh-configuration' >> .zshrc
 echo 'if [[ -e /usr/share/zsh/manjaro-zsh-config ]]; then' >> .zshrc
 echo 'source /usr/share/zsh/manjaro-zsh-config' >> .zshrc
 echo 'fi' >> .zshrc
 echo '# Use manjaro zsh prompt' >> .zshrc
 echo 'if [[ -e /usr/share/zsh/manjaro-zsh-prompt ]]; then' >> .zshrc
 echo 'source /usr/share/zsh/manjaro-zsh-prompt' >> .zshrc
 echo 'fi' >> .zshrc
