...
echo '/usr/local/bin/dvtws --port 989 --filter-tcp=443  --filter-tcp=80  --dpi-desync=fake,split2 --dpi-desync-split-pos=1,midsld  --dpi-desync-repeats=11 --dpi-desync-ttl=0 --dpi-desync-any-protocol --dpi-desync-cutoff=d4 --dpi-desync-fooling=badseq &' >> /etc/rc.firewall.my
